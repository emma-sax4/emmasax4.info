...
By the end of the night, I had pretty much decided to just do-away with the whole notion of a "set" and turn it into a "category" instead. Actually I like the word "collection" more than a "category", so in the codebase it's a "category", in my head it's a "collection", and to an end user of this site, they all look like "tags". So then, I did a _huge_ find/replace of the word `set` and replaced it with the word `category`.

Despite all of this work, now every single tag and category filters blog posts properly. The ending secret to getting pagination working on those pages is that I needed to save the URL of the filtered page: aka `/blog/welcome-to-kenya` or `/blog/tech`. And this way, we could append the page count number to the end of that: `/blog/welcome-to-kenya/4/` or `/blog/tech/2/`.
...
Haha! You thought the story was over, didn't you? Well... as soon as I merged in my pull request to add the new pagination, I realized something. That paragraph of the installation instructions that I _didn't_ finish? Well, the ending part of it warned that `jekyll-pagination-v2` is _not_ supported by GitHub Pages. This means that when somebody runs the code locally on their computer, they can use the `jekyll-pagination-v2` gem to their hearts' content, and the site will load properly. But GitHub's servers (which normally build the site for me), will not use the `jekyll-pagination-v2` gem, and there's no way to bundle it properly with that gem.
...
It turns out (surprise surprise) I'm not the first person to run into this issue. There's a <a href="https://github.com/sverrirs/jekyll-paginate-v2/issues/9" target="_blank">whole discussion</a> that's going on about this topic, and trying to integrate the gem with GitHub Pages eventually, but it hasn't made any progress lately. But, I did get some ideas from the discussion.

1. After I finish making any changes, I can commit my new source code to GitHub, bundle the project on my own computer, and then upload the bundled site to a _separate_ branch in GitHub and have GitHub Pages deploy only _that_ branch.
...
So, we could (in theory) make a change to the source code in GitHub, and then have Travis CI build my website and deploy it to GitHub Pages. And, it turns out that my site was already using Travis CI to build on every commit to the `master` branch (to test that the site builds properly). So, I only had to add a few lines to have Travis CI _also_ deploy my changes:
```
script:
...
* <a href="https://medium.com/@mcred/supercharge-github-pages-with-jekyll-and-travis-ci-699bc0bde075" target="_blank">medium.com: @mcred, supercharge github pages with jekyll and travis ci</a>
* <a href="https://docs.travis-ci.com/user/for-beginners/" target="_blank">docs.travis-ci.com: user, for beginners</a>
