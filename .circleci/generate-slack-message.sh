...
if [ -z $CIRCLE_PULL_REQUESTS ]; then # pull request does not exist
  ...
  if [ -z $CIRCLE_USERNAME ]; then # committer user does not exist
    m="Build #$CIRCLE_BUILD_NUM of $CIRCLE_PROJECT_USERNAME/$CIRCLE_PROJECT_REPONAME:\`$CIRCLE_BRANCH\` in PR [$PR_NUM]($CIRCLE_PULL_REQUEST)"

  else # committer user does exist
    m="Build #$CIRCLE_BUILD_NUM of $CIRCLE_PROJECT_USERNAME/$CIRCLE_PROJECT_REPONAME:\`$CIRCLE_BRANCH\` in PR [$PR_NUM]($CIRCLE_PULL_REQUEST) by $CIRCLE_USERNAME"
  fi
fi
